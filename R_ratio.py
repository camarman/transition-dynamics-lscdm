...
ax0.plot(z_ta_range, R_ratio_lscdm_1, color='#FFA500', ls='-', lw=3.5,
        label=r'$y_{\dagger}=0.60~(\rho_{\Lambda_{\rm s}}(a_{\rm ta}) > 0)$')
ax0.plot(z_ta_range, R_ratio_lscdm_2, color='#FF0000', ls='-', lw=3.5,
        label=r'$y_{\dagger}=0.70~(\rho_{\Lambda_{\rm s}}(a_{\rm ta}) > 0)$')
ax0.plot(z_ta_range, R_ratio_lscdm_3, color='#800000', ls='-', lw=3.5,
        label=r'$y_{\dagger}=0.80~(\rho_{\Lambda_{\rm s}}(a_{\rm ta}) > 0)$')

ax0.plot(z_ta_range, R_ratio_lscdm_4, color='#000080', ls='-', lw=3.5,
        label=r'$y_{\dagger}=1.01~(\rho_{\Lambda_{\rm s}}(a_{\rm ta}) < 0)$')
ax0.plot(z_ta_range, R_ratio_lscdm_5, color='#0000ff', ls='-', lw=3.5,
        label=r'$y_{\dagger}=1.05~(\rho_{\Lambda_{\rm s}}(a_{\rm ta}) < 0)$')
ax0.plot(z_ta_range, R_ratio_lscdm_6, color='#00ffff', ls='-', lw=3.5,
...
ax0.set_ylim(0.96, 1.06)

# Tick options
ax0.set_xticks([1.7, 2.0, 2.3, 2.6, 2.9, 3.2, 3.5, 3.8, 4.1, 4.4, 4.7])
ax0.set_yticks([0.96, 0.98, 1.00, 1.02, 1.04, 1.06])
